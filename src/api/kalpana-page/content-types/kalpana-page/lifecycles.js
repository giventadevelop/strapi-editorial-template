...
function assignTenantFromContext(event) {
  return (async () => {
...
async function ensureTenantAfterSave(uid, result) {
  ...
  try {
    ...
    strapi.log.warn(`Could not auto-assign tenant to ${uid}:`, err.message);
  }
}
...
module.exports = {
  beforeCreate(event) {
    return assignTenantFromContext(event);
  },
  async afterCreate(event) {
    await ensureTenantAfterSave(UID, event.result);
  },
