...
/**
 * Push Institutions entries from local Strapi to Strapi Cloud (production).
 ...
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/institutions-production-cloud-push-prd.html
 *
 * Options:
...
const PLURAL = 'institutions';
const LOCAL_FETCH_LIMIT = 1000;
...
async function getCloudInstitutionKeys(tenantIdFilter) {
  const map = new Map();
  let page = 1;
  const pageSize = 100;
  while (true) {
...
async function main() {
  ...
  if (list.length === 0) {
    console.log('No institution entries found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.');
    process.exit(0);
  }

  console.log('Push Institutions entries to Cloud');
  ...
  const cloudKeys = await getCloudInstitutionKeys(tenantIdFilter);
