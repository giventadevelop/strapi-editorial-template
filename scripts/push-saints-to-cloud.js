...
/**
 * Push Saints entries from local Strapi to Strapi Cloud (production).
 ...
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/saints-production-cloud-push-prd.html
 *
 * Options:
...
async function getCloudSaintKeys(tenantIdFilter) {
  const map = new Map();
  let page = 1;
  const pageSize = 100;
  while (true) {
...
async function main() {
  ...
  if (list.length === 0) {
    console.log('No saint entries found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.');
    process.exit(0);
  }

  console.log('Push Saints entries to Cloud');
  ...
  const cloudKeys = await getCloudSaintKeys(tenantIdFilter);
