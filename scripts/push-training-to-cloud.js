...
/**
 * Push Directory – Training programs from local Strapi to Strapi Cloud (production).
 ...
 *   set DRY_RUN=1&& npm run push:training-to-cloud -- --tenant-id=tenant_demo_002
 *   npm run push:training-to-cloud -- --tenant-id=tenant_demo_002
 *   npm run push:training-to-cloud -- --tenant-id=tenant_demo_002 --images-only
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/training-production-cloud-push-prd.html
 *
 * Options:
...
const PLURAL = 'training-programs';
...
async function getCloudTrainingKeys(tenantIdFilter) {
  const map = new Map();
  let page = 1;
  const pageSize = 100;
  while (true) {
...
async function main() {
  ...
  if (list.length === 0) {
    console.log('No training programs found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.');
    process.exit(0);
  }

  console.log('Push Training programs to Cloud');
  ...
  const cloudKeys = await getCloudTrainingKeys(tenantIdFilter);
