...
/**
 * Push Publications entries from local Strapi to Strapi Cloud (production).
 ...
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/publications-production-cloud-push-prd.html
 *
 * Options:
...
const PLURAL = 'publication-entries';
...
async function getCloudPublicationKeys(tenantIdFilter) {
  const map = new Map();
  let page = 1;
  const pageSize = 100;
  while (true) {
...
async function main() {
  ...
  if (list.length === 0) {
    console.log('No publication entries found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.');
    process.exit(0);
  }

  console.log('Push Publications entries to Cloud');
  ...
  const cloudKeys = await getCloudPublicationKeys(tenantIdFilter);
