...
/**
 * Push Ecumenical articles from local Strapi to Strapi Cloud (production).
 ...
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/ecumenical-production-cloud-push-prd.html
 *
 * Options:
...
const PLURAL = 'ecumenical-articles';
...
async function getCloudEcumenicalKeys(tenantIdFilter) {
  const map = new Map();
  let page = 1;
  const pageSize = 100;
  while (true) {
...
async function main() {
  ...
  if (list.length === 0) {
    console.log('No ecumenical articles found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.');
    process.exit(0);
  }

  console.log('Push Ecumenical articles to Cloud');
  ...
  const cloudKeys = await getCloudEcumenicalKeys(tenantIdFilter);
