...
/**
 * Push Directory – Spiritual Organisations from local Strapi to Strapi Cloud (production).
 ...
 *   set DRY_RUN=1&& npm run push:spiritual-organisations-to-cloud -- --tenant-id=tenant_demo_002
 *   npm run push:spiritual-organisations-to-cloud -- --tenant-id=tenant_demo_002
 *   npm run push:spiritual-organisations-to-cloud -- --tenant-id=tenant_demo_002 --images-only
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/spiritual-organisations-production-cloud-push-prd.html
 *
 * Options:
...
const PLURAL = 'spiritual-organisations';
...
async function getCloudSpiritualOrganisationKeys(tenantIdFilter) {
  const map = new Map();
  let page = 1;
  const pageSize = 100;
  while (true) {
...
async function main() {
  ...
  if (list.length === 0) {
    console.log(
      'No spiritual organisations found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.'
    );
    process.exit(0);
  }

  console.log('Push Spiritual Organisations to Cloud');
  ...
  const cloudKeys = await getCloudSpiritualOrganisationKeys(tenantIdFilter);
