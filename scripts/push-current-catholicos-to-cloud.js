...
/**
 * Push Current Catholicos entries from local Strapi to Strapi Cloud (production).
 ...
 *   DRY_RUN=1 node scripts/push-current-catholicos-to-cloud.js --tenant-id=tenant_demo_002
 *   node scripts/push-current-catholicos-to-cloud.js --tenant-id=tenant_demo_002
 *   node scripts/push-current-catholicos-to-cloud.js --tenant-id=tenant_demo_002 --images-only
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/current-catholicos-production-cloud-push-prd.html
 *
 * Options:
...
const PLURAL = 'current-catholicos-entries';
...
function buildPayload(doc, cloudTenantDocId) {
  return {
    name: doc.name,
    slug: doc.slug,
...
async function main() {
  ...
  if (list.length === 0) {
    console.log(
      'No Current Catholicos entries found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.'
    );
    process.exit(0);
  }

  console.log('Push Current Catholicos entries to Cloud');
  ...
  const cloudKeys = await getCloudEntryKeys(tenantIdFilter);
  ...
  for (const doc of list) {
    ...
    const payload = buildPayload(doc, cloudTenantDocId);
