...
/**
 * Push Catholicate entries from local Strapi to Strapi Cloud (production).
 ...
 *   DRY_RUN=1 node scripts/push-catholicate-to-cloud.js --tenant-id=tenant_demo_002
 *   node scripts/push-catholicate-to-cloud.js --tenant-id=tenant_demo_002
 *   node scripts/push-catholicate-to-cloud.js --tenant-id=tenant_demo_002 --images-only
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/catholicate-production-cloud-push-prd.html
 *
 * Options:
...
async function main() {
  ...
  if (list.length === 0) {
    console.log('No catholicate entries found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.');
    process.exit(0);
  }

  console.log('Push Catholicate entries to Cloud');
