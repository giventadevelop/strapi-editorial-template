...
/**
 * Push Holy Synod members from local Strapi to Strapi Cloud (production).
 ...
 *
 * Documentation:
 *   documentation/front_end_based_collection_types_import/production_cloud_data_push/holy-synod-production-cloud-push-prd.html
 *
 * Options:
...
const PLURAL = 'holy-synod-members';
...
async function getCloudHolySynodKeys(tenantIdFilter) {
  const map = new Map();
  let page = 1;
  const pageSize = 100;
  while (true) {
...
async function main() {
  ...
  if (list.length === 0) {
    console.log('No holy synod members found locally' + (tenantIdFilter ? ` for tenant ${tenantIdFilter}` : '') + '.');
    process.exit(0);
  }

  console.log('Push Holy Synod members to Cloud');
  ...
  const cloudKeys = await getCloudHolySynodKeys(tenantIdFilter);
